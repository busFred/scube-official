...
from fvdb.nn import VDBTensor, GELU

from scube.modules.diffusionmodules.openaimodel.util import checkpoint, conv_nd
...
class Attention(nn.Module):
    ...
    def forward(self, x: VDBTensor):
        ...
        for batch_idx in range(q.grid.grid_count):
            out.append(self._attention(q.data[batch_idx].jdata, k.data[batch_idx].jdata, v.data[batch_idx].jdata))
    ...
    def forward(self, x: VDBTensor, context: torch.Tensor=None, mask=None):
        ...
        for batch_idx in range(q.grid.grid_count):
            if exists(mask):
                ...
                out.append(self._attention(q.data[batch_idx].jdata, k[batch_idx], v[batch_idx], mask=mask[batch_idx:batch_idx+1]))
            else:
                out.append(self._attention(q.data[batch_idx].jdata, k[batch_idx], v[batch_idx]))
        out = fvdb.JaggedTensor(out)
        out = VDBTensor(x.grid, out, x.kmap)
        return self.to_out(out)

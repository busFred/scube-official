...
class ResBlock(TimestepModule):
    ...
    def forward(self, data: VDBTensor, emb: torch.Tensor,
        ...
        if data.grid.grid_count == 1:
            ...
            out_jagged_tensor = JaggedTensor(data_h.data.jdata) + JaggedTensor(self.skip_connection(data).data.jdata)
...
class AttentionBlock(nn.Module):
    ...
    def _forward(self, x: VDBTensor):
        ...
        if x.grid.grid_count == 1:
            # RuntimeError: batch indices' shape should match between this tensor and other tensor
            out_jagged_tensor = JaggedTensor(feature.data.jdata) + JaggedTensor(x.data.jdata)
